...
class BalanceAccount:
    ...
    def setupAutoPayment(
        self,
        payee: Payee.Payee,
        amount: float,
        freq: datetime.timedelta = datetime.timedelta(days=30),
    ):
                    ...
                    for account in main.AccountInterface.clientAcc:
                        ...
                        if account.phone == phone or account.e_mail == email:
                            etransfer = Etransfer.Etransfer(
                                self, amount, email, phone, str(self), str(account)
                            )  # The receiver argument is a string representation of the receiving account
                            self.withdraw(amount)
            ...
            for payee in self.master.payees:
                if (isinstance(payee, EtransferPayee.EtransferPayee) and payee.email == email):
                    for account in main.AccountInterface.clientAcc:
                        ...
                        etransfer = Etransfer.Etransfer(
                            self, amount, email, sender=str(self), receiver=email
                        )
                        self.withdraw(amount)
                        self.newTransaction(etransfer)
                        return etransfer
        ...
        # Case if just the phone was given
            ...
            for payee in self.master.payees:
                if (
                    ...
                    etransfer = Etransfer.Etransfer(
                        self, amount, phone=phone, sender=str(self), receiver=phone
                    )
                    self.withdraw(amount)
                    self.newTransaction(etransfer)
                    return etransfer
