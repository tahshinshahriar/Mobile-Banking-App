...
class ClientAccount(UserAccount.UserAccount):
    def __init__(
        ...
        self.creditAccounts = []
        self.balanceAccounts = (
            [self.chequingAccount] + self.savingsAccounts + self.creditAccounts
        )
    ...
    def requestMoney(self, amount, requestee):
        ...
        if requestee in main.AccountInterface.clientAcc:
            requestee.incomingRequests.append((self, amount))
            return (self, amount)
        return None
    ...
    def makeLoanPayment(
        self, account: BalanceAccount.BalanceAccount, amount: int, loan: Loan.Loan
    ):
